// =======================================================
// GLOBAL CONSTANTS / FRIENDLIES / TOP-LEVEL UI
// =======================================================
...
function openManageMatchesOverlay(matches) {
...
function ensureManageMatchesOverlay() {
  ...
  document
    .getElementById("close-manage-matches")
...
async function resolveOrCreatePlayerByName(name, { allowGuest = true } = {}) {
...
async function loadTournaments() {
  ...
  const [{ data: tournamentsData, error: tournamentsError }, { data: matchesData, error: matchesError }] =
    await Promise.all([
      supabase
        .from("tournaments")
        .select("id, name")
      ...
      supabase
        .from("matches")
        .select("id, tournament_id, match_date"),
    ]);
  ...
  matches.forEach((m) => {
    ...
    if (m.tournament_id && m.tournament_id !== FRIENDLIES_TOURNAMENT_ID) {
...
function wireManageMatchAdd() {
  ...
  btn.onclick = async () => {
    const p1 = document.getElementById("mm-p1").value.trim();
    const p2 = document.getElementById("mm-p2").value.trim();
    const dateVal = document.getElementById("mm-date").value;
    ...
    if (!p1 || !p2) {
      showErr("Please enter both player names.");
      return;
    }
...
if (!allowedPlayerIds.includes(p1Id) ||
      ...
      <div class="card">
        ...
        <div class="matches-scroll">
          ${matches.length
            ? matches.map(m => `
              <div class="match-row">
                <span>${m.player1?.name} v ${m.player2?.name}</span>
                <span class="muted">${formatDate(m.match_date)}</span>
              </div>
            `).join("")
...
function renderCountriesView(countries) {
  ...
  document.querySelectorAll(".country-card").forEach(card => {
    card.addEventListener("click", () => {
      ...
      renderCountryTournaments(country, countries[country], countries);
    });
  });
}
...
function bindOverviewTabs() {
  document.querySelectorAll(".tab-row .tab").forEach(tab => {
    tab.addEventListener("click", () => {
      ...
      ["overview", "standings", "fixtures", "results", "manage"].forEach(id => {
...
function buildTournamentPlayers(matches) {
  ...
  console.log(
    "[buildTournamentPlayers] players:",
    window.tournamentPlayers
  );
}
...
// Load ALL stages for manage view (not filtered to selected edition)
...
const showManage = canManageTournament(tournament);
  ...
  if (showManage) {
    ...
    const matchesCard =
      document.getElementById("manage-matches-card");
    if (matchesCard) {
      matchesCard.onclick = () => {
        openManageMatchesOverlay(matches);
      };
    }
  }
}
...
<div class="card manage-card clickable" id="manage-matches-card">
  ...
  <div class="manage-desc">
    Add and manage matches so they can be scored.
  </div>
</div>

  <div class="manage-actions">
    <button class="header-btn small" id="add-match-btn">
      + Add match
    </button>
  </div>

  <div id="manage-matches-list">
    <div class="empty-message">Loading matches…</div>
  </div>
</div>
  ...
  addMatchBtn.onclick = async () => {
    try {
      ...
      const allowedPlayerIds =
        (window.tournamentPlayers || []).map(p => p.id);
...
async function loadManageMatches() {
  ...
  const { editionId, stageId } = window.tournamentContext;

  if (!editionId || !stageId) {
    container.innerHTML =
      `<div class="empty-message">Select an edition and stage first.</div>`;
    return;
  }
  ...
  const { data: matches, error } = await supabase
    ...
    .select(`
      ...
      player2:player2_id ( id, name )
    `)
    .eq("tournament_id", window.currentTournamentId)
    .eq("edition_id", editionId)
    .eq("stage_id", stageId)
    .order("match_date", { ascending: true });
  ...
  if (!matches || matches.length === 0) {
    container.innerHTML =
      `<div class="empty-message">No matches created yet.</div>`;
    return;
  }

  container.innerHTML = matches.map(m => `
    <div class="manage-match-row" data-mid="${m.id}">
      <div class="match-players">
        ${m.player1?.name || "TBC"} vs ${m.player2?.name || "TBC"}
      </div>
      ...
      <div class="match-actions">
        <button class="icon-btn delete-match" data-id="${m.id}">
          🗑
        </button>
      </div>
    </div>
  ...
  // Delete handlers
  container.querySelectorAll(".delete-match").forEach(btn => {
    btn.onclick = async () => {
...
async function loadTournamentView(tournamentId) {
  ...
  renderDateBar(matchDates, (selectedDate) => {
    ...
    activeDateFilter = selectedDate || (matchDates.includes(today) ? today : null);
    renderMatchCards(
      matches,
      tournamentId,
      liveSetByMatch,
...
if (standingsArr.length === 0) {
  ...
  standingsHtml += `
    <table class="standings-table">
      ...
      <tbody>
        ${standingsArr.map((p, index) => `
          <tr data-pos="${index + 1}">
            <td class="pos" width=2.0em style="text-align:center;">${index + 1}</td>
            <td style="text-align:left;">${p.name}</td>
            <td style="text-align:center;">${p.played}</td>
            <td style="text-align:center;">${p.setsWon}</td>
            <td style="text-align:center;">${p.setsLost}</td>
